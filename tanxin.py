...
#这个函数有一个重大失误就是节点可以激活无限次，大实际上只能激活一次,即影响力传播一次
def computeRS(u):
    if u!=None:
...
#         print copyS
    ...
    while True:
        ...
        copyS.extend(list(set(active_node)))
    ...
    while True:
        ...
        for node in nodes:
            ...
            if node in S:
                continue
#             start1 = time.time()
            #这个值肯定得是大于0的最小也要是1(这是建立在实验概率不变的基础上的，但是每次进行实验的时候计算computeRS中的感染概率是一个随机值，所以前面的说法是错误的)
            curDs = computeRS(node)
            print str(curDs)
            dsu = curDs-ds
#             end1 = time.time()
