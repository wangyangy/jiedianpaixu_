...
def getTarget():
    ...
    # fp = open("data\\newdata\\2220\\number_number.txt",'r')
#     fp = open("data\\gouzaoshuju\\number_number.txt",'r')
    fp = open("data\\data_2019_1_11\\wiki-vote\\7066-nodes_number_number.txt",'r')
    k=0
    for line in fp.readlines():
        k=k+1
        if k%10000==0:
...
#                 #这里说明了k是和i,j均相连的节点
#                 if i in m[k]:
#                     temp+=((float)(Q[k])/(float)(C[i]))*((float)(Q[j])/(float)(C[k]))
#             res[i] += temp*temp
#             temp = 0.0
#         if res[i]==0.0:
...
#         print item[0]+":"+item[1]
    ...
    f = open("data\\data_2019_1_11\\wiki-vote\\7066-nodes_C.txt",'w')
    result = {}
    '''排序前十的节点'''
    for i in range(N):
#         if str(t[i][0])=='0':
